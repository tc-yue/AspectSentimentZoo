...
from sentiment_models.layers import ConnectAspectLayer, AtaeAttention, LocationAttentionLayer
...
class TAN(BaseModel):
    ...
    def build(self, embedding_matrix, aspect_embedding_matrix):
        ...
        target_augemented_embedding = ConnectAspectLayer()([word_embed, aspect_emb])
        alpha = LocationAttentionLayer(name='tan')(target_augemented_embedding)
        if self.config['bidirectional']:
            ...
            if self.config['rnn'] == 'gru':
                ...
                rnn_out = LSTM(self.config['rnn_output_size'],
                               ...
                               dropout=self.config['dropout_rate'],
                               recurrent_dropout=self.config['dropout_rate'])(word_embed)
        cr = dot([alpha, rnn_out], axes=1, name='attention_mul')
